...
import torch
import torch.nn as nn # new added
import torch.optim as optim # new added
import numpy as np
import random
...
def freeze_layers(model, num_layers=0):
    ...
    for name, param in model.named_parameters():
        ...
        # check the weight params in the convolution layer
        if "conv" in name and "weight" in name:
            # if a specified number of convolution layers have been frozen, exit the loop
            if frozen_count >= num_layers:
                break
            # freeze the parameters of the current convolution layer
            param.requires_grad = False
            # freeze count increase
            frozen_count += 1
        elif "conv" in name and "bias" in name:
            # the bias parameter of the frozen convolution layer
            param.requires_grad = False
...
def train(tr_loader, va_loader, te_loader, model, model_name, num_layers=0):
    ...
    learning_rate = 1e-3  # learning rate is 0.001

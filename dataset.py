...
class ImageStandardizer(object):
    ...
    def fit(self, X):
        ...
        self.image_mean = np.mean(X, axis=(0, 1, 2))  # Shape: (3,)
        
        # Calculate the standard deviation for each color channel (R, G, B)
        self.image_std = np.std(X, axis=(0, 1, 2))  # Shape: (3,)

    def transform(self, X):
        """Return standardized dataset given dataset X."""
        # TODO: Complete this function
        # Ensure mean and std have been computed
        if self.image_mean is None or self.image_std is None:
            raise ValueError("Must fit the data before calling transform.")
        
        # Standardize each channel by subtracting mean and dividing by std

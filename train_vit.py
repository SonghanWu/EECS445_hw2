...
def main():
    ...
    # Model
    model = ViT(
        num_patches=16,         # Appendix D
        num_blocks=2,           # Appendix D
        num_hidden=16,          # Appendix D
        num_heads=2,            # Appendix D
        num_classes=2,          # default setting is 2 categories
        chw_shape=(3, 64, 64)   # image shape, number of channels is 3, 64 x64 size
    )

    # TODO: define loss function, and optimizer
    criterion = nn.CrossEntropyLoss()
    learning_rate = 1e-3  # the learning rate specified

...
class TransformerEncoder(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # Apply Multi-Head Attention with residual connection and LayerNorm
        x = x + self.multi_head_attention(self.norm1(x))  # Residual connection and LayerNorm before Attention

        # Apply LayerNorm and MLP, then residual connection
        x = x + self.mlp(self.norm2(x))  # Residual connection and LayerNorm before MLP
        return x
...
class ViT(nn.Module):
    ...
    def forward(self, X: torch.Tensor) -> torch.Tensor:
        ...
        embedded_patches = self.patch_to_token(patches)  # use linear projection to embed each patch

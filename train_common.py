...
def early_stopping(stats, curr_count_to_patience, prev_val_loss):
    ...
    # get current validation losses (latest round of training)
    current_val_loss = stats[-1][1]

    # check to see if the validation loss is reduced
    if current_val_loss < prev_val_loss:
        # if the validation loss decreases, reset the counter and update the validation loss
        curr_count_to_patience = 0
        prev_val_loss = current_val_loss
    else:
        # if the validation loss does not decrease, the counter increases
